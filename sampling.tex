...
As input \nsc is given a query, the result of running this query on the dirty data, a dirt sample/clean sample pair.
...
However, this only considered data errors that transformed rows, not those that required new rows to be inserted or deleted.
...
\end{definition}
To apply this operation, we rewrite the \sumfunc, \countfunc, and \avgfunc queries using a selection with a \textbf{case} statement, and use the primary key (pk) that we defined in the previous section.
...
For sampling ratio $m$, we can estimate the query correction:
%\begin{lstlisting}[mathescape,basicstyle={\scriptsize}] 
\begin{lstlisting}[mathescape,basicstyle={\scriptsize}]
$\;\;\;\;$$\dans_{count,sum} = $ SELECT $ \sumfunc\big(*)/m$ FROM d;
\end{lstlisting}
For the \avgfunc query, we can calculate divide the corrections for \sumfunc and \countfunc:
\begin{lstlisting}[mathescape,basicstyle={\scriptsize}]
$\;\;\;\;$$\dans_{avg} = \frac{\dans_{sum}}{\dans_{cnt}}$;
\end{lstlisting}
To apply the correction, we take the stale query result and add the correction:
\begin{lstlisting}[mathescape,basicstyle={\scriptsize}]
$\;\;\;\;$$q(S') \approx q(S) + \dans_{avg,sum,count}$;
\end{lstlisting}
...
We can also correct stale Select queries with SVC.
...
\end{theorem}
\begin{proof}[Sketch]
The basic idea is that if we have two independent estimates, we want to average them to make a single estimate, and we know nothing about these estimates before hand, the 
optimal thing to do is to take an equally weighted combination. See \reminder{TR} for the full version of the proof.
\end{proof}
\iffalse
...
Refer to our extended paper for more details \reminder{TR}.
\end{proof}
...
Refer to the extended technical report for the details on the bootstrap procedure \reminder{TR}.
...
However, we can only acheive bound that has a slightly different interpretation than the confidence intervals seen before.
We can calculate the probability that a larger (or smaller) element exists in the unsampled view.
Refer to the extended technical report for the details on \minfunc and \maxfunc \reminder{TR}.
