...
An unbiased result estimate means that expected value of the estimate over all samples of the same size is $q(S')$ \footnote{The \avgfunc query is considered conditionally unbiased in some works, however, this difference is largely notational and does not affect any subsequent bounds.}.
