...
\label{correction}
In this section, we discuss how to correct stale query results using sample update patterns. We first present our correction query processing in Section~\ref{subsec:correct-principle} and then discuss the error bars and optimality of query corrections in Section~\ref{subsec:correct-practical}. \reminder{No Section 5.2?}
...
WHERE Condition(A);
\end{lstlisting}
%For ease of presentation, we assume the query does not have a group-by clause.
%But our methods can be easily extended to a group-by query by treating each group key as an additional condition of the query. 
...
Let us consider the case where there is no index on the materialized view. \reminder{Why do you make the assumption? It seems that the following statement still holds even without the assumtion.}
...
For \aggview, since there are both updates and insertions, we do introduce an overhead. \reminder{It's not clear (1) what do you mean by overhead; (2) why do we have overhead in cases of updates and insertions.}
In this case, our query has to scan both the stale view and the update pattern sample.
If the sampling ratio is small, then this overhead will be small as well. \reminder{I think the point we want to make here that the cost is still applying the query to the stale view and sampled update patterns. While this is not guaranteed to be smaller than full maintenance, since sample is usually much smaller than the stale view, the overhead is quite small. Furthermore, when \aggview is added many new groups, it is still possible that we are better than full maintenance. }
