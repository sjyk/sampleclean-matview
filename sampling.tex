...
Given a stale view and a query on the view, let \ans denote the stale result of the query. The goal of \cqp is to use the sample update patterns w.r.t the view to correct \ans. To achieve this goal, our basic idea is to first obtain a \emph{delta query result} (denoted by $\dans$), by rewriting the query against the sample update patterns, and then combine \ans and $\dans$ into a corrected query result. 
...
\begin{lstlisting} 
SELECT f(a) FROM Table WHERE Predicate(A);
\end{lstlisting}
where \texttt{f($\cdot$)} is an aggregation function (i.e., $\avgfunc$, $\sumfunc$, $\countfunc$), \texttt{a} is an attribute, and \texttt{A} is an attribute set. 
But note that our methods can be easily extended to a group-by query by treating each group key as a predicate.

 focus on a non query without a group-by clause, i.e., 

But note that 

Without loss of  ease of presentation, we consider the query  focus on the query in the form of:
