...
\reminder{<}Corrections from the traditional AQP approach which would be to apply a query directly to the up-to-date sample view \reminder{> unclear}.
...
If $R_1.a_1$ or $R_2.a_1$ is $\emptyset$, then they are respectively represented as~zero.
\end{definition}
...
\sloppy
In many cases correcting query results approximately is more accurate than an AQP-style direct estimate. 
