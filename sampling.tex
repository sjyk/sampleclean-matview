...
\begin{table*}[ht!]
\caption{Query Result Semantics \reminder{Have you referred Table 1 somewhere in the text?}}  % title of Table
...
\reminder{Do you need to talk about sample mean? It seems that you do not use it in the following estimation of sum, avg, and count.}
...
This makes the a pointwise subtraction between two samples ill-defned. \reminder{typo and syntax error?}
...
Then, we rewrite the \sumfunc, \countfunc, and \avgfunc queries using as \reminder{syntax error?} a selection with a \textbf{case} statement, and use the primary key (pk) for the view we defined in the previous section.
...
\noindent For \sumfunc, and \avgfunc: \reminder{Is it correct to write \avgfunc in this form? Furthermore, it seems that you never use it in the following since \avgfunc is derived from \sumfunc and \countfunc.}
\begin{lstlisting} [mathescape]
...
To find these bounds, we can rewrite the terms $\dans$ as a mean value of random variables. \reminder{How? I think you can move the description on how to rewrite as a mean value (in the 2nd paragraph of Sec 5.1) here. }
...
We apply the query to stale view, and then we apply the sample estimate of the query to up-to-date sample. \reminder{what's the relationship between the previous sentence and the following text. }
...
\reminder{I would suggest to first say something like some queries do not have unbiased estimates, thus the question is how to get the correction along with the error bar for these queries. After that, you show how you do it.}
...
\reminder{Is it the estimate (and bound) of SAQP or correction?}
...
If $X$ is a random variable with mean $\mu_x$ and variance $var(X)$, then the probability that $X$ is larger than a constant $a$ \reminder{a has been used as an attribute before. Change it to a different symbol?} plus the mean is:
\[
