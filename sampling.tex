...
These procedures are extensions of the query processing approaces proposed in SampleClean \cite{wang1999sample}, extending it to missing data and aggregates beyond \sumfunc, \countfunc, and \avgfunc.
...
In the general case, this distribution is only described by the set of all of its values (i.e no smaller parameterized representation).
This distribution is called a non-parameteric distribution and over this class of distribution the sample mean is an MVUE for the population mean.
...
However, in the parameteric case, it is not true in general.
In other words, if we knew a priori that our data is from a discrete parameteric distribution we could devise
...
However, even for many discrete parameteric distributions, the sample mean estimators are MVUEs, eg. poisson, bernouilli, binomial, hypergeometric.
...
\begin{enumerate}[noitemsep]
\item Take subsamples $\widehat{S}_{sub}'$ and $\widehat{S}_{sub}$ with replacement from $\widehat{S}'$ and $\widehat{S}$ respectively
...
\minfunc and \maxfunc fall into their own category since this is a cannonical case where bootstrap fails.
