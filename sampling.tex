...
$c = \mathbb{E}\big[e_q(\hat{S'})\big] -q(s) $. \reminder{$q(s)$ or $q(S)$}
...
There now have been new log records inserted into the Log table making the old resu lt stale, and suppose we are working with a sampling ratio of 5\%.

\reminder{It's better to follow the algorithm that you described above to estimate query results. For example, in the algorithm of SVC+CORR, you applied SVC+AQP twice, but I did not see this kind of process in the example}

For SVC+AQP, we count the number of videos in the sample that currently have counts greater than 100 and scale that result by 20. \reminder{Tell the reader where 20 comes from, i.e., $\frac{1}{5\%} = 20$ }\reminder{Give the final answer returned by SVC+AQP.}
...
We extrapolate that 40 \reminder{Tell the reader where 40 comes from, i.e., $2\times \frac{1}{5\%} = 40$} new videos throughout the view should now be included in the count.
This means that we should correct the old result by $40$ resulting in the estimate of $45+40 = 85$. \reminder{This example is duplicated with Example 2.}
\end{example}
...
Then, the error in the estimate is normally distributed\footnote{We can include a finite population correction if our relation size is small $\frac{N-k}{N-1}$. \reminder{This looks not very clear. }}:
\[
...
instead of a predicate. \reminder{The case statement is not well defined. I did not see it in the following queries. }
...
trans:= SELECT pk, 1.0/m $\cdot$ attr $\cdot$ cond(*) as trans_attr
        FROM sample 
\end{lstlisting} 
For \avgfunc since there is no scaling we do not need to re-write: \reminder{It seems more natural to show count right after sum, but show avg in the end. }
\begin{lstlisting}[mathescape,basicstyle={\scriptsize}]
...
To calculate the confidence intervals for \svcnospace+CORR we have to look at the statistics of $c$ \reminder{Do you need $c$ here?} the difference, i.e., $c = q(S) - q(S')$, from a sample.
...
\[c = q(S - S'),\]
where $-$ is the row-by-row difference between $S$ and $S'$. \reminder{ $-$ is not well defined. Think about whether it is possible to avoid having $-$ here.}
...
\item Record the difference $s\cdot(q(\widehat{S}_{sub}')-q(\widehat{S}_{sub}))$ \reminder{Make sure this is correct. For example, if it is avg, you don't need $s$ here. }
\item Return to 1, for k iterations.
\item Return the $100-\alpha$\% and the $\alpha$\% percentile of the distribution of results. \reminder{Since you are trying to find a 95\% confidence interval, you can set $\alpha$\% to 2.5\%. }
\end{enumerate}
