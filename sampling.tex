...
\label{correction}
In this section, we discuss how to correct stale query results using sample update patterns. We first present the principle of this query processing in Section~\ref{subsec:correct-principle} and then describe how to implement it practically in Section~\ref{subsec:correct-practical}.
...
SELECT $\countfunc(a)/\sumfunc(a)/\avgfunc(a)$ FROM View 
WHERE Condition(A);
\end{lstlisting}
But note that our methods can be easily extended to a group-by query by treating each group key as an addition condition. 
...
Since both of the update patterns of \spview and \fjview only consist of inserted records, their correcting query processing phases can be taken as the same. Given the above query, to get its delta query result, we can simply run the query on the sample update patterns. The following shows the rewritten queries for different aggregation functions.  
...
For the \countfunc and \sumfunc queries, we only need to compute $\dans_{count}$ and $\dans_{sum}$, respectively. But for the \avgfunc query, in addition to $\dans_{avg}$, we also need $\dans_{cnt}$ for query correction.



\begin{table}\renewcommand{\arraystretch}{1.2}
\caption{Correcting a stale query result}\label{tbl:query-correct} \small \centering
...
\begin{tabular}[t]{|l||c|c|c|}
  ...
  %\multicolumn{4}{|c|}{A set $\Lambda$ of $\ar$s} \\ \hline\hline
   \hline
                & \specialcell{\spview \& \fjview} & \aggview \\ \hline
