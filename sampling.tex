\section{Query Processing on Cleaned Views}
\label{correction}
...
We first describe two result estimation procedures, which we denote as \svcnospace+CORR and \svcnospace+AQP. \reminder{I forgot when you first used AQP and CORR. But make sure that readers can understand what AQP and CORR stand for.}
...
We apply the query to the sample and scale it by $N$: \reminder{what is $N$?}
\[
...
\noindent\textbf{SVC+CORR: } Given a clean sample $\widehat{S}'$, its corresponding dirty sample $\widehat{S}$, a query q, and a scaling factor $N$: \reminder{$N$ or $s$?}
\begin{enumerate}[noitemsep]
\item Apply \svcnospace+AQP to $\widehat{S}'$ \reminder{This looks not very formal.}
...
\reminder{It would be better to add a detailed example here to explain how SVC+SQP and SVC+CORR work. You can use a SUM (or COUNT/AVG) query, and show how to rewrite it against a sample data for SVC+SQP and SVC+CORR, respectively.}
...
\reminder{I did not quite understand this section. (1) If it is analysis of result estimates, why not incorperate the biasness analysis above into this section.  (2) It also seems that you want Sec 5.3 and Sec 5.4 to be the subsections of sec 5.2. }
...
\reminder{From this section, I know how to get confidence intervals, but I don't know why you do it in this way, why it is correct, why it is challenging, and why we need row-by-row difference, }
...
For \sumfunc: \reminder{``*" has two meaning in the following query, which is a little confusing.}
...
\reminder{What's our contribution compared to typical bootstrap techniques?}
...
\item Record the difference $e_q(\widehat{S}_{sub}')-e_q(\widehat{S}_{sub})$ \reminder{Have you defined $e_q$ before?}
\item Return to 1, for k iterations.
\item Return the 97.5\% and the 2.5\% percentile of the distribution of results.
\end{enumerate}
...
\subsection{Example}
\reminder{This example is only about result estimation. I would suggest to put it before Sec 5.2.}
Returning to our example \tbl{visitView}, suppose a user wants to know how many videos have received more than 100 views.
