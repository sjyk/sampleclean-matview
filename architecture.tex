...
The focus of this paper is analytic workloads where the typical query is a group by aggregate.
...
We jointly refer to these relations as the delta relations $\partial \mathcal{D}$.
...
\noindent \textbf{Staleness as Data Error: } The consequences of staleness are dirty data incorrect, missing, and superfluous rows.
...
\begin{itemize}[noitemsep] \sloppy
	...
	\item \textbf{Superfluous: } Superfluous row errors are the set of rows (identified by the primary key) that exist in the stale view but not the up-to-date view : \[\{ \forall u \in S : \not\exists u \in S' \}\]
\end{itemize}
...
Let $q$ be an aggregate query of the following form \footnote{\scriptsize For simplity, we exclude the group by clause for all queries in the paper, as it can be modeled as part of the \textsf{Condition}.}:
