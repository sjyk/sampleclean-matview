...
SVC corrects stale query results on three classes of materialized views, Select-Project, Foreign-Key, and Aggregation:
...
All sampling-based systems are limited in the ways that they can address general Select queries.
We provided limited support for Select queries, we can return the stale result with any additional rows that appear in the sample.
However, since we can bound aggregation queries, we can bound how many rows are missing from the result which is a \countfunc query. 
