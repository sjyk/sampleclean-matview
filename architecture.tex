...
In this paper, we present our approach with insertions to the base table as the only type of update, see
\reminder{Extended Paper} for more details on how to model deletions.
Our framework cleans stale query results on three classes of materialized views, Select-Project, Foreign-Key, and Aggregation:
...
We focus on deriving corrections for the three most common aggregation queries on the view \footnote{For ease of presentation, we assume the query does not have a group-by clause, which can be easily extended by treating each group key as an additional condition of the query.} : 
\begin{lstlisting} [mathescape]
SELECT $\sumfunc(a)/\countfunc(a)/\avgfunc(a)$ FROM View 
WHERE Condition(A);
\end{lstlisting}
For these queries, we bound the results in confidence intervals with the Central Limit Theorem and prove optimality of our approach with respect to estimate variance (Section \ref{subsec:correct-practical}).

For general Select-Project queries on the view, we can still derive a bounded correction using the Chebyshev Inequality. 
While the corrections are unbiased, this bound is not as tight as the CLT bound for the aggregation queries, and is not 
optimal with respect to variance. 
Characterizing which queries can be answered accurately using a sample is a subject of active research\cite{agarwalknowing}, and we hope to explore the problem of unbiased correction and optimality beyond (SUM, COUNT, AVG) in future work.
...
Suppose, we cleaned a random sample of dirty rows processing only as many of the new insertions as necessary.
For aggregate queries, such as \sumfunc, \countfunc, and \avgfunc, we can then estimate from this sample how much this cleaning changes the query results.
...
The diagram depicts the following components: (1) as updates arrive we sample the ``update pattern". 
...
\begin{figure}[t] 
\centering
 \includegraphics[scale=0.33]{figs/sample-clean-example.png}\label{example-1}
