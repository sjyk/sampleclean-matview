...
We apply SVC to three classes of materialized views, Select-Project, Foreign-Key, and Aggregation: \reminder{It would be better to say something about other types of views.}
\vspace{1em}

\noindent\textbf{Select-Project Views (\spview): } These views are defined by Select-Project
expressions of the following form:

\reminder{Maybe you can make the notations of views be consistent with the notations of queries.}
...
We can model \update as a \delete of the old record then an \insertion of a new record with updated values. Insertion-only workloads are increasing common \reminder{Tim will find a citation}.
...
In this paper, we focus on deriving corrections for the three most common \reminder{Maybe saying ``three commonly used" is less argulable. } aggregation queries on the view\footnote{\scriptsize For ease of presentation, we assume the query does not have a group-by clause, which can be easily extended by treating each group key as an additional condition of the query.} : 
...
See Section \label{outlier} \reminder{?} for details on this component.
...
For example, if our sampling ratio is 5\%, our system maintains a sample of this view. \reminder{Avoid saying that we maintain a sample of view. We actually maintain update patterns for a sample of view.}
