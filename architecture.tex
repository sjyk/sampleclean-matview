...
View definitions are composed of standard relational algebra expressions: Select ($\sigma_{\phi}$), Project ($\Pi$), Join ($\bowtie$), Aggregation ($\gamma$), Union ($\cup$), and Intersection ($\cap$) \reminder{Do we consider Difference (-) in the paper?}. 
...
We use the ``hat'' notation (e.g., $\widehat{S}, \widehat{\mathcal{M}(\cdot)}$) to denote sampled relations and sampled relational expressions. \reminder{ It's unclear to me what $\widehat{\mathcal{M}(\cdot)}$ is? Did you use it somewhere?}
We say the relation $\hat{S}$ \reminder{Make the notation consisitent, i.e., $\widehat{S}$ and $\hat{S}$} is a \emph{uniform sample} of $S$ if
...
\reminder{The maintenance strategy part of the example tells me how to maintain a MV, but does not help me understand the definition of the maintenance strategy. It's better to follow the maintenance strategy definition and show what it looks like for the example MV. For example, you can say what  $S,\mathcal{D}, \partial D$ are, and how to compose them using relational expressions to get the maintenance strategy.}
...
\item Take the full outer join of the ``delta view" with the stale view visitView (equality on videoId, language, ownerId \reminder{no duration?}).
...
view $\widehat{S'}$.
\item \emph{(Problem 2. Query Result Estimation)} For an aggregate query $q$, we use $\widehat{S'}$ \reminder{Is $\widehat{S'}$ enough to estimate query results?} to estimate the up-to-date result.
...
takes the database $\mathcal{D}$ (\tbl{Log} and \tbl{Video}), and the delta relation $\partial \mathcal{D}$ (\tbl{LogIns}) as input and updates the visit counts for the sample. \reminder{Be more clear about the output of $\mathcal{C}$ }

In Query Result Estimation (Problem 2), we take the result of running $\mathcal{C}$, which is a sample of the up-to-date visit counts \reminder{If you clarify the output of $\mathcal{C}$ above, then you don't need this sentence here.}, and estimate our above query.
...
This means that we should correct the old result by $40$ resulting in the estimate of $45+40 = 85$. \reminder{This is duplicate with Example 5. It also looks a little werid that you only show SVC+CORR but not SVC+AQP. }
\end{example}
