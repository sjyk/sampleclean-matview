...
There may be many different strategies to apply updates (maintenance) to $S$ based on the insertions and deletions, and we denote the up-to-date view as $S'$.
...
It is a function of the database $\mathcal{D}$, the stale view $S$, and all the insertion and delection relations $\{\Delta R_i\} \cup \{\nabla R_i\}$.
\end{definition}

%In this work, we ignore \textbf{null} types and assume that each tuple in the relation has non-null attribute values.
In this work, we look at materialized views and maintenance strategies composed of the following relational expressions:
...
\end{itemize}
As a convenient conceptualization of composable relational exrepssions, in this paper, we often expressed them in a well-studied tree form called an \emph{expression tree}.

To make the concept of a maintenance strategy concrete, we show an example view based on our running example dataset in Figure \ref{exexpr}.
Our example view joins the Log table with the Video table and counts the visits for each video grouped by id.
If new records have been added to \tbl{Log} and \tbl{Video}, then the maintenance strategy consists of the following expressions: (1) create a ``delta view" by applying the view definition to \tbl{LogIns}, (2) update the view by taking the join of the ``delta view" with \tbl{countView}, (3) project the result by incrementing \tbl{visitCount}. 
