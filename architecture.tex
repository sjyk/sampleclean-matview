...
\item $\gamma_{e}(R)$: Aggregation of R grouped by the relational expression $e(R)$.\reminder{Is it formal? Where are group-by keys, aggregate functions, aggregate attributes in the notation?}\footnote{A special case of this operator is $\delta$ the deduplication operator.}\vspace{-.45em}
...
\reminder{The section raised two concerns: 1. why uniform sampling; 2. why not reservoir sampling. Try to address them.}
...
\reminder{What's the point of this section? Do you intend to talk about sampling error v.s. staleness?}
...
If we are given a materialized view $S$ and we know the base relations have had insertions and deletions $U$ \reminder{U?}, then given the operators defined earlier there are three possible types of error:
\begin{itemize}
...
\reminder{What's the difference between the problem that this section tries to address and the problem that the last paragraph of Sec 3.2.1 mentions?}
...
In Section \ref{correction}\reminder{4?}, address this challenge by defining a set of push down rules for $\eta$ to maximize the operators that benefit from the sampling.

\subsubsection{Query Correction}

Given a query $q$ which has been applied to the stale view $q(S)$; thus the result is stale.
We want to calculate a $c$ \reminder{What is c? Do you assume the output of $q(S)$ is a single value?} such that $q(S)+c$ is fresh using the defition described previously.
...
\reminder{An idea just came into my mind. Do we really need correspondence for normalizedSC? Will that work if we run a query on a dirty sample and run a query on a cleaned sample, where the two samples have no relationship, and then get the difference?}

\reminder{Correspondence is a property that sampling has to follow. Why do you define it here?}
...
Like similar restrictions in other sampled-based systems \cite{agarwalknowing}, there are restrictions on the queries $q$ on the view that we can answer. \reminder{You can first show the form of the queries that the SampleClean work can support. Then, we say in this work, we extend it from two perspectives: a) aggregation functions; b) selection queries. }
...
The correction $c$ \reminder{c is not well defined. I think you can avoid saying it here.} in the previous subsection is derived from a sample.
...
We illustrate this setup in Figure \ref{sys-arch}. \reminder{Figure 1?}
