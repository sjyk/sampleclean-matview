...
We look at relational expressions for the view $S_{def}$ composed of the following operators:
...
We formalize the maintenance process as a \emph{maintenance strategy} $\mathcal{M}$.
...
GROUP BY 
	videoId;
\end{lstlisting}
If new records have been added to Log and Video, then the maintenance strategy consists of the following expressions: (1) create a ``delta view": apply the view definition to LogInserts, (2) update the view: full outer join of the ``delta view" with browserCountView incrementing viewCount.
...
Returning to our running example, suppose a user wants to know how many videos have received more than 100 views.
\begin{lstlisting} 
SELECT COUNT(1)
FROM browserCountView
WHERE viewCount > 100;
\end{lstlisting}
...
For example, if our sampling ratio is 5\%, that means for 5\% of the videos (distinct videoID's), we update just the view counts of those videos.
