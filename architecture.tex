...
To navigate this spectrum, only option for the user is to change the maintenance period from eg. daily, hourly, or immediately.
However, this parameter is constrained by what can be processed by the system and can be only tuned by prior experience.
...
For aggregation queries (SUM, COUNT, AVG) on the materialized view, we can bound these estimates in confidence intervals which allow the user to understand how far they are from the exact answer even in the presence of unexpected updates.
Thus, our results are never stale; they have some inaccuracy introduced by the sampling ratio but in expectation they are correct.
