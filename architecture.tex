...
We motivated how frequent incremental maintenance of materialized views can be challenging given system resource constraints.
Materialized view maintenace techniques lie on a spectrum of freshness and performance.
To navigate this spectrum, only option for the user is to change the maintenance period from eg. daily, hourly, or immediately. \reminder{Is this the only option? Be careful of saying "only"}
However, this parameter is constrained by what can be processed by the system and can be only tuned by prior experience. \reminder{The parameter of sampling ratio seems also have this issue. }
Unexpected updates can easily lead to results that are more stale than the user anticipated.

Modeling this as data cleaning problem can allow us to change the spectrum from freshness vs. performance to accuracy vs. performance.
Rather using the maintenance period to select how many updates to propagate at a time, we sample the queued up updates.
Sampling gives us an additional degree of freedom to satisfy the system constraints for any maintenance period.
We use a sample of up-to-date (clean) data to estimate how much that data changes the stale query result, thus deriving a correction term which can ``clean" the query.
For aggregation queries (SUM, COUNT, AVG) \reminder{Use the commands for \sumfunc, \countfunc, and \avgfunc} on the materialized view, we can bound these estimates in confidence intervals which allow the user to understand how far they are from the exact answer even in the presence of unexpected updates.
