...
Finally, we overview the system architecture and discuss a numerical example of how this works in practice \reminder{inconsistent; no architecture; no example}. 
...
A view $S$ is considered \emph{stale} when there exist insertions or deletions to its base relations.
We jointly refer to these relations as the delta relations $\partial \mathcal{D}$. \reminder{$\partial \mathcal{D}$ is not well defined. It sounds that they refer to the base relations who have updates.}
...
\[
S' = \mathcal{M}(S,\partial D)
\]

\vspace{.25em}

\noindent \textbf{Staleness as Data Error: } The consequences of staleness are dirty data incorrect, missing, and superfluous rows\reminder{syntactic error?}. 
...
In Figure \ref{exexpr}, we illustrate the example view from the previous section.
Our example view joins the Log table with the Video table and counts the visits for each video grouped by videoId, language, and ownerId.
The primary key of Log is sessionId and the primary key of Video is videoId.
If new records have been added to Log staged in the temporary table LogIns, then the expressions are needed to update the view (Figure \ref{exexpr}):}

\vspace{-.55em}
\begin{enumerate}[noitemsep]
\item \emph{Create a ``delta view'' by applying the view definition to LogIns. That is, calculate the visit count per video, language, and ownerId on the new logs.}
\item \emph{Take the full outer join (equality on videoId, language, ownerId) of the ``delta view'' and the stale view.}
\item \emph{Apply the generalized projection operator to increment visitCount (adding the delta view visitCount  and the stale visitCount where NULL is treated as zero).}
\end{enumerate}
\vspace{-.5em}

\reminder{This example did a good job to explain maintenance strategy; But it did not help to understand other concepts; You can either have one giant example that tells a story connecting all concepts together or add a small example to the end of each concept.}
...
\subsection{Stale Sample View Cleaning}
\reminder{1. Make the problem formalization as a definition; 2. Try to think how to use a figure or an example to explain this definition.}
The first problem addressed in this paper is how to clean a sample of the stale materialized view.
We are given a stale materialized view $S$ and a maintenance strategy $\mathcal{M}$.
We have a sample of this view $\widehat{S}$ with ratio $m$, the insertion and deletion set $\partial \mathcal{D}$, and a maintenance strategy $\mathcal{M}$.
We want to find a relational expression $\mathcal{C}$ that takes the above relations as input and returns a clean sample $\widehat{S}'$, a sample of the up-to-date view, with sampling ratio $m$:
\[
\widehat{S}' = \mathcal{C}(\widehat{S},\partial \mathcal{D})
\]

\reminder{Do you need $\mathcal{D}$? This makes me more confused about the definition of $\partial \mathcal{D}$. }

\subsection{Query Result Estimation}
\reminder{1. Make the problem formalization as a definition; 2. Try to think how to use a figure or an example to explain this definition.}
...
q(S') = q(S) + c
\]
Our objective is to estimate $q(S')$ using $q(S)$, $\widehat{S}$, and $\widehat{S}'$.
We propose two query result estimation approaches: \svcnospace+AQP which estimates $q(S')$ directly from a sample, and
\svcnospace+CORR which estimates $c$ from the differences between $\widehat{S}$ and $\widehat{S}'$.

\subsection{\svc Workflow}
\reminder{(a). This workflow does not match Figure 1; Maybe you need to draw a new figure. (b) I think it is better to put this section before Secs 3.2 and 3.3. Based on this workflow, you can abstract two challenging problems which are Secs 3.2 and 3.3. (c) I think Sec 3.2-3.4 are very important. We need to write them as clear as possible and as formal as possible. }
We summarize the system in Figure \ref{sys-arch} in our introduction.
Formally, the workflow of \svc is:
\begin{enumerate}[noitemsep]
\item Given a view $S$.
\item $\mathcal{M}$ defines the maintenance strategy that updates $S$ to $S'$ at each maintenance period.
\item The view $S$ is stale between periodic maintenance.
\item We find an expression $\mathcal{C}$ that updates a sample of the materialized view $\widehat{S}$ to 
	$\widehat{S}'$.
\item For an aggregate query $q$ on the view $S$, we use $\widehat{S'}$ to estimate the up-to-date result.
