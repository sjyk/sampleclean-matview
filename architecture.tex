...
\svc returns a bounded approximation for aggregate queries result \reminder{aggregate queries result?} on stale MVs for a flexible additional maintenance cost.
...
At the leaves of the tree are all of the \emph{base relations} for a view. \reminder{``At ... are" seems have a syntax error.}
...
We denote the entire row for that primary key as a selection $\sigma_u(S)$. \reminder{This can not determine the entire row for that primary key. For example, think about what $\sigma_{sessionId}(Log)$ means?}
...
We use the ``hat'' notation (e.g., $\widehat{S}$) to denote sampled relations and sampled relational expressions\reminder{What is ``sampled relational expression"?}.
