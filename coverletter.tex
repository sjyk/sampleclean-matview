...
The sampling technique proposed in Section 4 of our paper could be used to approximate the data cleaning techniques in \cite{DBLP:journals/pvldb/0002M13, DBLP:journals/pvldb/0002M13, DBLP:conf/sigmod/ChalamallaIOP14}.\end{displayquote}
...
{\bf Responses:} We have addressed all of the details from reviewer comments in the reviewer section of the cover letter. To summarize, we cited the algorithm that we used for Incremental View Maintenance which is the change-table (also called a delta table e.g \cite{DBLP:journals/vldb/KochAKNNLS14}) algorithm described in “Incremental maintenance of aggregate and outer join expressions” by Gupta et al. We further clarified our contribution for the two compared query processing approaches, SVC+CORR and SVC+AQP. Both techniques use our Stale View Cleaning technique but have a different result estimation procedure. There were also reviewer concerns about selectivity (for which we added a theoretical analysis) and update size (for which we clarified the experiment in which those results can be seen).
...
Even when view maintenance should be relatively inexepensive such as in Twitter's Rainbird \cite{rainbird} which is restricted to commutative/associative aggregates, 
in these systems, maintenance is not completely eager with some degree of batching ($\sim 60$ seconds).
...
There are three primary limitations for \svc: class of queries, types of materialized views, and the specification of maintenance strategy. \reminder{it seems that you forgot to mention the third limitation, the specification of maintenance strategy, below.  }
...
We used this approach \reminder{which approach: top-k or stdev? } in our experiments (Section 7.2.4) and list the tradeoff between outlier index size and improvements in query result accuracy.
...
We have also clarified that none of our experiments before 7.2.4 used an outlier index. The caveat is that these experiments were done with moderately skewed data with Zipfian parameter = 2, if this parameter is set to 4 then the 75\% quartile query estimation error is nearly 20\% (Figure 11). \reminder{I did not find Fig 11.} 
...
\begin{displayquote}
\noindent\textbf{\svcnospace+AQP: } We maintain a sample of the materialized view using \svc and estimate the result with SVC+AQP\reminder{You defined \svcnospace+AQP recursively. Does SVC+AQP represents a query estimation method or maintenance+query estimation?}. 

\noindent\textbf{\svcnospace+CORR: } We maintain a sample of the materialized view using \svc and process queries on the view using \svcnospace+CORR\reminder{The same issue as above.}.
\end{displayquote}
...
{\bf Responses:} Due to space restrictions, we have removed our discussion of support for non-aggregate queries as it is not essential to our work. SVC can support error estimation for SELECT queries (with low selectivity) by estimating how many row are missing due to sampling. \reminder{I didn't get the point of the last sentence.}
...
However, if we do know more about the distribution, this optimality is not true in general\reminder{It sounds that it is not optimal in most of the cases. Maybe a better way is to say ``this optimality is not always true"}. The intuitive problem is that if there are a small number of parameters that completely describe the discrete distribution there might be a way to reconstruct the distribution from those parameters rather than estimating the mean. As a simple counter example, if we knew our attributes \reminder{attributes?} were exactly on a line, a sample size of two is sufficient to answer any aggregate query. However, even for many parametric distributions, the sample mean estimators are still MVUEs, e.g., poisson, bernouilli, binomial, normal, exponential. It is often difficult and unknown in many cases to derive an MVUE other than a sample mean. Furthermore, the sample mean is unbiased for all distribution \reminder{What do you mean by ``all distribution"}, but it is often the case that alternative MVUEs are biased when the data is not exactly from correct model family (such as our example of the line). Our approach is valid for any choice of estimator if one exists, even though we do the analysis for sample mean estimators and this is the setting in which that estimator is optimal.\end{displayquote}
