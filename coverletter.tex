...
This problem addressed in Meliou et al. is hard since they do not assume a model for the errors.
...
Furthermore, in real deployments, it is common to use the same infrastructure to maintain multiple materialized views (along with other analytics tasks) \reminder{say something about distributed environment} adding further contention to computational resources and reducing overall maintenance throughput. 
...
While we only experiment with first-order delta-table IVM in this paper, it is a straight forward extension to apply \svc to higher-order maintenance. \reminder{If this is straightforward, why did we not apply \svc to higher-order maintenance?}
\svc can also benefit from SQL query compilation as we can compile the sample maintenance strategy to make \svc even faster. \reminder{there is no such sentence in Section 9.}
\end{displayquote}
