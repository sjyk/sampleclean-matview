...
query accuracy for a flexible additional \reminder{Remove additional?} maintenance cost that can
...
It is also unclear how our approach interacts with user defined functions or recursive table expressions. \reminder{It reads like SVC has so many limitations. We need to think about how to better organize this section. Maybe you can start by saying there are some queries and views that SVC cannot support. Then, we give one example for either case, and say why it is challenging for sampling. }
\end{displayquote}
...
For these queries, we can model selectivity as a reduction of effective sample size $kp$ \reminder{what is k?}.
...
We generated 100 random \sumfunc, \avgfunc, and \countfunc queries for each view. \reminder{Make tense consistent}
\end{displayquote}
...
\end{displayquote}
\reminder{Do you need to show No maintenance (Stale) and Incremental View Maintenance (IVM)?}
...
{\bf Responses:} We have also incorporated the reviewer’s space saving suggestions by revising the presentation of the relational algebra. \reminder{How about the other suggestions?}
