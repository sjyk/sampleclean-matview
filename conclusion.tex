...
%While our experiments show that \svc works for a variety of applications, there are a few limitations which we summarize in this section.
...
Sampled-based methods are fundamentally limited in the way they can support ``point lookup" queries that select a single row, and we believe we can address this problem with new results in non-parameteric machine learning instead of using single-parameter estimators.
In particular, we are interested in deeper exploration of the multiple MV setting.
