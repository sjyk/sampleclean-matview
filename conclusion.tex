...
between accuracy and performance. \reminder{I would suggest to remove the flexible trade-off since SAQP can also give such trade-off. I think the main conclusion is that SVC can accurately correct the stale query results while require orders of magnitude less time than full maintenance}
...
To support general selection queries, \reminder{remove this?} we will have to extend our query correction to predict and attribute value via a regression.
...
\reminder{Multiple view settings: automating sampling ratio selection; learning update patterns for multiple view (see if there are some opportunities to share computation); select the best view to correct a given query result }
