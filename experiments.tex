...
Refer to our extended paper on more details about the experimental setup \cite{technicalReport}. \reminder{this paragraph looks werid. Is it for the the entire sec 7.1 or just complex views. Choose a better place to say this sentence.}
...
\noindent\textbf{\svcnospace+CORR: } We maintain a sample of the materialized view using \svc and process queries on the view using the correction with \reminder{which?} applies the AQP to both the clean and dirty samples, and uses both estimates to correct a stale query result.
...
\includegraphics[scale=0.14]{exp/msj_3.pdf}\vspace{-1.5em}
 \caption{For a fixed sampling ratio of 10\% and update size of 10\% (1GB), we generate 100 of each TPCD parameterized queries and answer the queries using the stale materialized view, \svcnospace+CORR, and \svcnospace+AQP. We plot the median relative error for each query. \reminder{It would be better to replace SVC+Corr with SVC+CORR in all figures.}\label{exp-1-acc}}\vspace{-.5em}
\end{figure}

\begin{figure}[t]
\centering
 \includegraphics[scale=0.13]{exp/msj_4.pdf}
 \includegraphics[scale=0.13]{exp/msj_6.pdf}\vspace{-1.0em}
  \caption{(a) For a fixed sampling ratio of 10\% and update size of 10\% (1GB), we measure the total time incremental maintenance + query time. (b) \svcnospace+CORR is more accurate than \svcnospace+AQP until a break even point. \reminder{$10->10\%$ in Fig 6(a)}\vspace{-1em} \label{exp-1-total}}
\end{figure}
...
\centering
 ...
 \caption{(a) For 1GB update size, we compare maintenance time and accuracy of \svc with a 10\% sample on different views. V21 and V22 do not benefit as much from \svc due to nested query structures. (b) For a 10\% sample size and 10\% update size, \svcnospace+CORR is more accurate than \svcnospace+AQP and No Maintenance.  \reminder{It might be better  to not use red to represent SVC 10\% (Figure 7(a)) since red represents SVC+Corr in Figure 7(b)}\label{exp3-acc}}\vspace{-1.5em}
\end{figure}
...
\textbf{Performance:} Figure \ref{exp3-acc} shows the maintenance time for a 10\% sample compared to the full view.
...
While there is an overhead, it is still small compared to the gains made by sampling the maintenance strategy. \reminder{It would be better to add some clarification in this section for Reviewer 1's comment on outlier indexing (E2-3)}
...
\centering
 ...
 \caption{(a) We compare the maintenance time of \svc with a 10\% sample and full incremental maintenance, and find that as with TPCD \svc saves significant maintenance time. (b) We also compare \svcnospace+CORR to \svcnospace+AQP and No Maintenance and find it is more accurate. \reminder{(1) red color issue (the same as Fig 7); change 10 to 10\%} \label{conv-1}}\vspace{-1.5em}
\end{figure}
...
V5 is the most expensive to maintain due to its nested query and V2 is a single level group by aggregate. \reminder{It would be better to say what you learnt from this experiment otherwise reviews will be still confused about the reason of having this experiment here. (1) our approach can be eaily applied to a distributed environment without sacrificing performance gain; (2) we can solve real problems!; (3) maybe other insights. }
