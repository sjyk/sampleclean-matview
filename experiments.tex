...
\reminder{I would suggest to move the content of the following paragraph to each coresponding section.}
...
\centering
 ...
 \caption{For each of the three views listed above, we plot the accuracy for 5\% samples compared to SAQP and a stale baseline. We also plot the maintenance time for each of the views in comparison to full incremental maintenance for a 1\%, 5\% and 10\% sample. \reminder{The x-axis does not show the ``\% of inserted"; Should we use SVC instead of SC?; Yellow (SC 1\%) is hard to see; Make the lines look different when people print our paper in black-white.  }}
\end{figure*}

\subsection{Experimental Setting}
\subsubsection{TPCD-Skew}
The first dataset, TPCD-Skew dataset \reminder{Find the ref to the program}, is based on the Transaction Processing Council's benchmark
...
Then, we randomly generated aggregation queries \reminder{please give the details that how we generate the queries} and evaluated them for different sample sizes, different update rates, and different parameter settings for the Zipfian distribution.
...
\reminder{I would suggest to merge Sec 7.2 and 7.7 into the  section of ``Accuracy" and merge Sec 7.3 and 7.4 and 7.5 into the section of ``efficiency". You can use the subtitle as below. }

{\noindent \bf Update Rate and Accuracy :}

\reminder{do we need an experiment of Data Skew v.s. Accuray}
...
\reminder{It seems that you forgot to compare with Stale error. }
...
For a batch of updates, we evaluate how long it takes to incrementally maintain a view compared to maintaining a sample. \reminder{Check the whole paper and try to avoid saying that we are maintaining a sample view since this topic has been investigated since 1992 (Maintenance of materialized views of sampling queries). Unlike prior works, we are actually ``sampling update patterns". For example, \spview and \fjview only require to sample inserted records; \aggview requires to know how an old record is changed to a new one. }
For the join view, we build an index on the foreign key of the view. \reminder{How about the index on \aggview?}
...
Furthermore, there are diminishing returns with small sample sizes as overheads start becoming significant. \reminder{It seems that this sentence is saying the issue of sampling? If so, is ``furthermore" an proper word used here?}
...
If our updates only contain insertions, then for Select-Project and Foreign-Key join views \reminder{Use the command \spview etc}, since we derive a correction from a sample of the updates, we are guaranteed to have a reduced query time.
...
answer the query. 
In our first experiment, \reminder{first? where is the second?}we set the sample size to 5\% and evaluate the aggregation view listed earlier for 5000000 inserted records. 
In (Figure \ref{exp10overheads}), \reminder{double check the labels of all figure} we show that average query time is small compared to maintenance time, and furthermore, 
...
\centering
 ...
 \caption{The overhead introduced by correcting a query is small compared to the overall maintenance time. \reminder{Why do you choose 50\%? I would suggest to choose 1\%, 5\%, 10\% as Figure 4. Why does the correction time not change by varying the sampling ratio? }}
\end{figure}
...
For a such a view,\reminder{typo?} the only cost for maintenance is a scan of the data and evaluating the predicate. 
Sampling saves on predicate evaluation but introduces the overhead of random number generation. \reminder{maybe you can refer to Sec 4.2.1 or Table 1}
...
Initially, the random number generation adds about 5\% \reminder{Can I get the number from the figure 6?} overhead, but as the cost of evaluating the benefits increase.
...
\hspace{-2.5em}
 ...
 \caption{As the views definitions become more complex our approach gives increasing gains. For simplest view (selection no predicate), the overhead is small (5\%). \reminder{For the y-axis of figures, may be speedup only looks more clear than \%speedup, e.g., 15x v.s. 1500 \%; Maybe it's better to remove ``full incremental" and add a dotted line whose y-axis label is 1x; Please double the results for the right figure. Why is the speedup for l\_orderkey that small?}}
\end{figure}
...
We varied the Zipfian parameter from (0,1,2,3,4) and measured the average improvement in accuracy. \reminder{What is the number of indexed outliers?}
...
We found that small indicies were sufficient for good accuracy and thus the additional overhead was marginal and orders of magnitude smaller than sample maintenance time. \reminder{Where is the maintenance time? It would be better to add into the figure. }

\begin{figure}[ht!]
\label{exp7outlier}
\hspace{-4em}
\includegraphics[scale=0.21]{exp/exp6-outlier-full.eps}
 \caption{Outlier indexing greatly improves accuracy of our approach especially in skewed datasets for a small overhead of building the index and ensuring those rows are in the view.; Why \reminder{(1) For the left most figure, I think it's better to show how the ourlier index improve those very badly results. Let's discuss it. (2) The order of the figures are inconsistent with the text. }}
\end{figure}

\subsubsection{Distribution of Query Error}

\reminder{It's weird to put this section here. I would suggest to move the ``Accuracy" section; Furthermore, it's not easy to tell from the figure the point that you want to make. Let's discuss it.}
...
\reminder{I think another point you want to show in the figure is that our approach only needs a very small sample to get better results than staleness.}
...
\hspace{-3em}
 ...
 \caption{As we increase the sample size, the relative error goes down following a $\frac{1}{\sqrt{k}}$ rate. \reminder{What is k?} SAQP also follows the same rate so the lines will never cross by only changing the sample size.}
\end{figure}
...
All three of these views are aggregation views, and for confidentiality, we exclude the actual definitions of the views. \reminder{Use a more descriptive name for view 1, 2, 3? For example, you can use \aggview (Large, Medium, Small)}
...
\includegraphics[scale=0.23]{exp/conviva_efficiency_accuracy.eps}
 \caption{We chose three views that represented the distribution of all views well in terms of both accuracy and maintenance time. \reminder{Why is the error \% so small? It is inconsistent with the result in Figure 11.}}
\end{figure}
...
This cluster had enough memory to hold all of the materialized views in memory. \reminder{Be careful of saying that. We can assume all the updates are loaded into memory, but we cannot assume all the MVs are in memory.}
