...
\textbf{Data Cube: } \reminder{Aggregate View? I am a little worried to say it is a cube because a typical definition of cube is a set of views that contain not only base cube but also all roll up queries.} Another application which is common in industry analytics is data cube.
...
The specification of this data cube and the queries are listed in the appendix. \reminder{Remember to change appendix to TR in our VLDB submission}

\textbf{Queries as Views: \reminder{Complex Views?}} Finally, to test the generality of SVC, we treat each of the 22 queries in TPCD as a materialized view.
We take the TPCH schema and denormalize the database, and treat each of the TPCH queries as views of this denormalized schema. \reminder{Give some high-level reasons, e.g., we remove queries that have a small query result set, etc. }
10 out of the 22 can benefit from SVC, and the reasons why queries were excluded is listed in the appendix.
\reminder{It would be better to say something about what these views look like and why they are more challenging than join view and aggregate view }
...
We generated 100 random aggregate queries\reminder{what are predicates, group-by attrs, and aggregate functions?} for each view.
We detail the incremental maintenance procedures in ??. \reminder{?}

\subsubsection{Distributed Experimental Setup} \reminder{Why is it a subsection of sec 8.1?}
...
There is a SQL interface which transforms the RDD's using Map-Reduce chains \reminder{Grammer error?}.
As RDD's\reminder{Grammer error?} are an immutable data structure, any maintenance must be done synchronously.
...
\reminder{For the conviva dataset, we need to be careful about what we say for privacy concerns. I will mark those sentences with [privacy].}
We evaluate SVC on a 1TB dataset of logs from Conviva \cite{conviva}.
...
\item Incremental View Maintenance (IVM): Applying maintaining the full materialized view. \reminder{hard to understand}
\item SAQP: Maintaining a sample of the materialized view (using SVC) but estimating the result with AQP rather than using 
a correction. \reminder{SAQP means SVC + AQP?}
...
\end{itemize}
Since both SAQP and SVC have sampling parameters, we denote a sample size of $x \% $ as SVC-x and SAQP-x, respectively. \reminder{Please check the label names in each figure. Sometimes, it shows SVC-10\%, IVM-10\%, SVC-10 10\%.}
...
To illustrate how SVC gives the user access to this new tradeoff space, we will illustrate that SVC is more accurate than SAQP and No maintenance as a baseline \reminder{Is SAQP a baseline?}; but is less computationally intensive than full IVM. 
...
\includegraphics[scale=0.16]{exp/msj_3.pdf}
 \caption{We generate 100 of each TPCD query type and process it using the stale materialized view, SVC, and SAQP. We plot the median relative error for each query type. SVC is more accurate than SAQP and No Maintenance. \reminder{Add stale after No Maintainance in Sec 8.1.2? i.e., No maintenance (Stale) }\label{exp-1-acc}}
\end{figure}

\begin{figure}[t]
\centering
 \includegraphics[scale=0.16]{exp/msj_4.pdf}
  \caption{For a fixed sampling ratio of 10\% we vary the update rate and plot the total time (maintenance + query execution) time. While SVC has an increased query execution time in comparison to SAQP, this overhead is small compared to the savings in maintenance time compared to full incremental maintenance. \reminder{I am confused about this figure. 1) what is the black bar (SAQP?); 2) what's the query time: a single query or 100 queries? 3) If I understand collectly, does this figure show that maintenance time is equal to the time of running 4-5 queries? } \label{exp-1-total}}
\end{figure}
...
In black \reminder{It's hard to tell black from red in a black-white paper}, we note the time for full IVM. 
...
For small update sizes, the speedup is far from ideal (5.3x at 250MB \reminder{I cannot find 250MB in the figure. What's the sampling ratio for 250MB?}) but as the updates get larger the efficiency improves and in fact we observe super linearity (> 10x) at larger sizes.
This is likely {likely? any other possible reason? } due to the fact that while the base relations have a join-index, we do not create an index on the updates \reminder{why?}, making the join very expensive if there are large number of new records. \reminder{I would suggest to revise the reason accordingly.}
...
We found that SVC was 11.7x \reminder{on average?} more accurate than the stale baseline, and 3.1x more accurate than applying SAQP to the sample.
...
While both techniques are sensitive to skew, we find that the gap between SVC and SAQP widens for more skewed datasets. \reminder{Is it possible to give an intuitive reason for that?}
...
\centering
 ...
 \caption{(a) We run the same experiment but replace the \sumfunc query with a median query. We find that similarly SVC is more accurate. (b) However, bounding the median correction with bootstrap is expensive but that overhead is small relative to total savings. \reminder{Why do you put SAQP-AVG into the figure but not SAQP-Median? What does the result estimation in IVM mean?}\label{exp2-median}}
\end{figure}
...
This is because both of those views contain nested structures \reminder{Why do nested structures block pushdown? What are algebraic operators?}which block the pushdown of our sampling.
...
\centering
 ...
 \caption{(a) For one view V3 and 1GB of updates, we plot the 75\% quartile error with different techniques as we vary the skewness of the data. We find that SVC with an outlier index of size 100 is the most accurate. \reminder{I think you don't need to show Stale, but need to add SAQP+Outlier.} (b) While the outlier index adds an overhead this is small relative to the total maintenance time. \label{exp5-oi} \reminder{Q---> V}}
\end{figure}
...
We find that four views: V3, V5, V10, V15, can benefit from this index with our push-up rules. \reminder{Is it because other views do not have l\_extendedprice or some operators block push-up rules? }
...
\reminder{I would suggest to remove this section because it raised more questons than benefits. 1) I/O cost sounds vague because view mainenance may also involve I/O cost; 2) It's hard to explain why the blue line in Figure 13(a) went down. In fact, I think the main point we want to say is we consider the case that the I/O cost of for scanning updates isis much cheaper than the other cost of view maintenance (either one view or multiple views). That's why view maintenance is expensive. You can say it in the experimental setting.}
...
This dataset was given to us in denormalized form.\reminder{``given to us" sounds informatl. You only need to describe what the dataset look like.}
...
While, we cannot give the details of the queries, we can present some of the high-level characteristics. \reminder{It seems that you have already given a high-level description of the queries in Sec 8.1.1. Remove this part?}

\reminder{The followings may have some [Privacy] issues. }
\begin{itemize} 
...
\centering
 ...
 \caption{(a) Spark RDD's are most efficient when updated in batches. As batch sizes increase the system throughput increases. \reminder{y-axis (e.g., 1.E+06) in Figure 15(a) looks weird. How do people usually represent it?} (b) The penalty for running maintenance in multiple threads reduces as batch sizes increase. \reminder{What do you mean by ``penalty" and ``slow down"?} \label{conv-2}}
\end{figure}

We devised an end-to-end experiment simulating a real integration with periodic maintenance.
However, unlike the MySQL case, Apache Spark \reminder{I think we need to explain why we use spark instead of distributed database systems either here or in the experimental setting. For example, you can say Spark is super popular right now. But in real-world applications it has some limitations (e.g., immutability, synchrony, etc), which make spark become even more expensive to maintain materialized views.  In the experiment, we show how Spark coupled with SVC can overcome the limitations, and achieve a better trade-off, and so on. } does not support selective updates and insertions as the ``views" are immutable.
...
Throughputs for small batches are nearly 10x smaller than the throughputs for the larger batches. \reminder{Do you think we need to say something about Spark Streaming?}
...
We chose these parameters in the following way. \reminder{It is better to give an overview on what's the goal of the experiment and how you design the experiment.}
...
Larger batch sizes are more amenable to parallel computation. \reminder{What's the sampling ratio for SVC? }
...
Using this, we set a throughput demand for both periodic maintenance and svc+periodic maintenance. \reminder{Why can you decide a throughput demand based on Figure 15(b)?}
...
These competing objective lead to an optimal sample size of 3\% for V2 and 6\% for V5. \reminder{sample size --> sampling ratio?}
