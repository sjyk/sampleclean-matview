...
The only terms in the correction that are not deterministic is $\dans$ and $\dans_{cnt}$. \reminder{I think you can remove $\dans_{cnt}$ from here. You may misunderstand my previous reminder. What I mean is that for \avgfunc of \spview and \fjview, we need to estimate both $\dans_{cnt}$ and $\dans_{avg}$. In this situation, can we still bound the query result?}
