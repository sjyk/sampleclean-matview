...
To illstrate the challenges in deriving $\mathcal{C}$, we present two naive solutions to this problem that will not work.
...
This is because to acheive a sample of $S'$, we need to ensure that for each $s \in S'$ all contributing rows in subexpressions to $s$ are also sampled. 
...
For the relational expressiosn defined in the previous section, this method will always work.
However, in the future, if we expand the allowed set of relational expressions there are some limitations which we defer to futur work.

\subsection{Hashing Operator}
\label{push}
Propsition \ref{provalg} gives an procedure of determining the set of rows that contribute to $r$. 
We now explore how we can design a sampling technique to guarantee that all of the rows in Propsition \ref{provalg} are sampled if $r$ is sampled.
