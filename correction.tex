...
The rows with incorrect aggregates appear with probability 0 in the unsampled view \reminder{I did not understand this sentence}. 

To get a uniform sample of a view, the main problem is that for every row sampled in the view, our sampling technique needs to include all of the rows in relations lower in the tree \reminder{``lower in the tree" sounds informal.} that could possibly contribute to its materialization.
...
\item $\Pi_{(a_1,...,a_k)}(R)$: If the projection includes the primary key of R then the primary key of the projection is the same as the primary key of R. Otherwise it is \textsf{null}. \reminder{Why is it null? We should include the primary key even if it is not projected.} \vspace{-.45em}
...
In the subsequent sections, we assume that the Primary Key for all expressions is never null. \reminder{Why do you need this assumption? You have made this assumption in the Base Case of Def 4.}
...
However, it is important to note that for some of the expressions, notably joins and aggregates \reminder{Why do aggregates block pushing down.}, tracability \reminder{traceability?} is more complex.
...
\item $\gamma_{f,e}(R)$: Blocks $\eta$ in general. If the group by expression can be decomposed into $e(R) = a, e'(R)$ then pushdown is possible. \reminder{Why is it blocked? If we define group-by keys as a set of attributes, it can always be pushed down. }\vspace{-.45em}
...
\subsection{Example}
\reminder{I think this is a very important example. Here are my suggestions: 1) Explain how to obtain the primary key of the view (step by step) using the rules defined in Sec 4.2; 2) Explain how to push down the hash operator (step by step) using the rules defined in 4.3. 3) Add figures if you think they are necessary. }
