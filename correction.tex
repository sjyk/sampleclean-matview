...
However, in the future, if we expand the allowed set of relational expressions there are some limitations which we defer to future work. \reminder{This sentence is not very clear. Which relational expressions did you refer to? How can you overcome the limitations?}
...
\reminder{It's not clear to me that how you use the PK generation procedure in Def 2 to get the PK of the above view. It might be better to draw an expression tree and show how each rule (base case, join, agg, proj) is applied. If the view is complex to draw, you can simplify the view. The main point of this example is to tell the reviewer that PK is not our assumption.}
...
the new view with the additional attributes. \reminder{Why do you need to say they are equivalent?}
...
\centering
 ...
 \caption{Applying the rules described in Section \ref{push}, we illustrate how to optimize the sampling of our example maintenance strategy. \reminder{In the figure, is it visitView or countView?} \label{exexpr2}}\vspace{-1em}
\end{figure}
...
Then, we reach another point where we hash the equality join key allowing us to push down the sampling to the relations \tbl{LogIns} and \tbl{Video}. \reminder{It seems that you cannot put the hash operator to the LogIns table because the table does not have ownerId, language, duration attributes. }
\end{example}
...
\item Sampling of Missing Rows: $I = \{\forall u \not\in S : u \not \in S\}$, \reminder{$I = \{\forall u \not\in S : u \in S'\}$ ?} \[\mathbb{E}(\mid I \cup \widehat{S'} \mid) = m\mid I \mid \] \reminder{$\mathbb{E}(\mid I \cap \widehat{S'} \mid)$ ?}
\item Key Preservation \reminder{Key Preservation for Updated Rows?}: For all $u\in \widehat{S}$ and not in $D$ or $I$, $u\in \widehat{S}'$.
\end{itemize}
\vspace{-.25em}
