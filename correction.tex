...
We present first two naive solutions to this problem that will not work. 
...
First, we could trivially apply $\mathcal{M}$ to the entire stale view $S$ and update it to $S'$, and then sample.
...
Let $A_R$ be the primary key of $R$ and $A_U$ be the primary key of $U$.
For a row $r\in R$, define $r(A_R)$ to be the value of the primary key.
Similarly, for a row $u\in U$, define $u(A_U)$ to be the value of the primary key.
$p_U(r)$ (the rows in U that contribute to r) is determined as follows: 
(1) if $A_R \subseteq A_U$ then 
$\{u \in U: u(A_R) = r(A_R)\}$, (2) if $A_R \not \subseteq A_U$ then
return $U$.
\end{proposition}
We want to guarantee that all of the rows in Proposition \ref{provalg} are sampled if $r$ is sampled.
...
{\noindent \textbf{Limitations: }} There are two basic cases where push down does not always work: joins and group by aggregates. 
...
The result of this hash operator push down on $\mathcal{M}$ is the cleaning expression $\mathcal{C}$. 
