...
Intuitively, update patterns represent the update information involved in a stale view. With such information, we can easily transform the stale view to the corresponding up-to-date view. There are three possible update patterns. (1)~Insert a new record into a view;
...
For \spview, the only changes of the view are inserting some new records, thus its update patterns only involve insertion.  
For instance, Figure~\ref{fig:update-pattern}(a-1) shows an example of \spview. When the base table \tbl{Log} is updated, there might be some new records whose \field{userAgent} field contains "Mozilla", which should be added into the \spview accordingly. Figure~\ref{fig:update-pattern}(a-2) illustrates the update patterns for the \spview. The figure means that in order to obtain the up-to-date \spview, we should insert $r_1$ and $r_2$ into the original \spview.
...
\begin{tabular}[t]{|l||c | c||c|c||c|c|}
  ...
  \bf{Delta View} & $\textrm{cost}_{pred}(n)$ & \specialcell{$\textrm{cost}_{rand}(n)$ + \\$ \textrm{cost}_{pred}(\alpha \cdot n)$}  & $\textrm{cost}_{join}(n)$ & \specialcell{$ \textrm{cost}_{rand}(n)$ + \\ $\textrm{cost}_{join}(\alpha \cdot n)$} &
  \specialcell{$\textrm{cost}_{group}(n)$ \\+ $\textrm{cost}_{agg}(\delta_{v})$} &
   \specialcell{$\textrm{cost}_{hash}(n)$ + \\$\textrm{cost}_{group}(\alpha \cdot n)$ + \\ $\textrm{cost}_{agg}(\alpha \cdot \delta_{v})$}        \\ \hline
  \bf{Refresh}    & $\textrm{cost}_{write}(\delta_v)$ & $\textrm{cost}_{write}(\alpha \cdot \delta_v)$  & $\textrm{cost}_{write}(\delta_v)$ & $\textrm{cost}_{write}(\alpha \cdot \delta_v)$ &  $\textrm{cost}_{apply}(\delta_v)$ & $\textrm{cost}_{apply}(\alpha \cdot \delta_v)$\\ \hline
\end{tabular}
\end{table*}
\vspace{.5em}


{\noindent \bf Cost Analysis.} Let $n = |\triangle T|$ be the number of inserted records, $v$ be the cardinality of the stale view, $\delta_v$ be the cardinality of the delta view, and $\alpha$ be the sampling ratio. Table~\ref{tbl:cost-analysis} compares the cost of our update-pattern sampling techniques (denoted by Sampling) with the corresponding cost of incremental view maintenance (denoted by Maintenance). The table does not include the I/O cost of loading the updates into memory since in both Sampling and Maintenance we only need to load the updates once and amortize that I/O cost over all views.
...
\hashfunc{\textrm{``group key"}} \leq \alpha.
\end{equation}
Obviously, the maintained group keys are a random sample of all the group keys with the sampling ratio of~$\alpha$.
...
Incremental view maintenance has to apply $\delta_v$ records to the entire view while our sampling approach only needs to apply $\alpha \cdot \delta_v$ records to the sample view. 
If there is an index in the view, we can 
determine which records are new insertions and which correspond to existing records in constant time. Therefore, our sampling technique can reduce the refresh cost from $\cost_{apply}(\delta_v)$ to $\cost_{apply}(\alpha\cdot \delta_v)$.
