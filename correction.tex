...
However, for general maintenance strategies, $\mathcal{M}$ does not commute with sampling. \reminder{I am confused why you have this subsubsection to separate the explanation of this alternative method. }
...
\reminder{Why do you need to define Primary Key Provenance?}
...
\reminder{I highly recommend to add an example here to explain how to compute a primary key for a MV using our rules. This example can also help reviewers to understand primary key is not an assumption that we enforce for a MV.}
...
When applied to a stale sample of a view \reminder{only the stale sample?}, it produces an up-to-date sample:
\[
\widehat{S}' = \mathcal{C}(\widehat{S},\partial \mathcal{D})
\]

\subsection{Corresponding Samples}
\reminder{I am still a little worried about this section because (1) in the proble formulation, you said $\widehat{S}'$ has to be a random sample; but here, the sample you generated depends on $\widehat{S}$. Is it still a random sample; (2) why does this sample give you the best answer? why not generate others?}
...
We will see how this property can be leveraged to improve query estimation accuracy. \reminder{ref sec 5.3.2? }

\reminder{Is it a definition or property? }
...
\vspace{-.25em}
\begin{itemize}[noitemsep]
\item Uniformity: Both $\widehat{S'}$ and $\widehat{S}$ have sampling ratio of $m$ which means the expected value of the size of the samples is a fraction $m$ of the total size of the view: \[\mathbb{E}(\frac{\mid\widehat{S}\mid}{\mid S \mid}) = \mathbb{E}(\frac{\mid\widehat{S}'\mid}{\mid S' \mid}) = m\] \reminder{I think you can remove this point and say in the beginning that ``$\widehat{S'}$ and $\widehat{S}$ are uniform samples of $S'$ and $S$ with a sampling ratio of $m$"}
\item Removal of Superfluous Rows: $D = \{\forall r \in \widehat{S} \wedge r \not\in S'\}$, $D \cup \widehat{S'} = \emptyset$ \reminder{$\cup$?}
...
\reminder{Always give one sentence in the begining or the end of a paragraph to say what's the point of this paragraph. Do you want to say our method can gurantee to get a coresponding sample that you defined above? }
...
\centering
 ...
 \caption{Applying the rules described in Section \ref{push}, we illustrate how to optimize the sampling of our example maintenance strategy. \reminder{Add more margin between the two figures.} \label{exexpr2}}\vspace{-1.75em}
\end{figure}
