...
The intuition is straight-forward\reminder{This sounds that our approach is trivial.}, if we have some way of keep track of which tuples are in our sample, we only have to do as much work as necessary to maintain those tuples. \reminder{The intuition is not very clear. }
...
The first challenge is \emph{correspondence}\reminder{not well defined}.  
For example, if we take a random sample of the stale view and a random sample of the updates \reminder{what is ``updates"?} independently; the rows in $\hat{S}$ and $\hat{S'}$ will not match up. 
Alternatively, we could sample $\hat{S}$ \reminder{syntax error?} and trace back through the view and try to figure out the correct changes.
However, without expensive indexing this will be infeasible.


\reminder{Think about how you come up with this hashing idea, and add it before this paragraph.}
To address this challenge, without the use of lookup tables\reminder{what is ``lookup tables"?}, we add the following operator $\eta_{a_1, m}(R)$ which is the \textbf{hash} operator. 
...
This definition is without loss of generality for uniform hash function, as if we have a hash function whose range is the set of integers (as implemented in MySQL or Apache Hive) we can take the absolute value and divide by the maximum integer mapping this range back $[0,1]$. \reminder{We can discuss these implementation details in the experiment}

Suppose we have a technique to ensure that an attribute (e.g. a primary key) is consistent \reminder{not well defined} between $S$ and $S'$, then this hash operator is sufficient to ensure that $\hat{S}$ and $\hat{S'}$ correspond.
...
The only pre-condition is that we have an attribute that is consistent, and we will formalize that in the next section.
...
\item $\Pi_{p,(a_2,...,a_k)}(R)$: Preserves lineage of R if it includes $p$ \reminder{This is different from the notation in Sec 2.1.1. } \vspace{-.45em}
\item $\bowtie_{\phi (r1,r2)}(R_1,R_2)$: Resets lineage. The primary key of the result is the union of the linages of the relations $p= p_1 \cup p_2$. \reminder{What is $p$?}
...
\subsection{More About Joins} \reminder{merge this section to sec 4.3.}
