\section{Extensions}
\reminder{add some text to give an overview and motivation of the two subsections}
\subsection{Select Queries}\label{sec:sel}
We can correct stale Selection queries with SVC. \reminder{selection or select? Be consistent}
...
To quantify the approximation error, we can rewrite the Select query as \countfunc to get an estimate of the up-to-date size of the result; giving us a bound on the number of missing rows. \reminder{Is bound a good word? Why not use estimation?}
...
In comparison to no maintenance, this approach gives a less stale result. \reminder{You forget to mention the advantage of ``bounds".}
...
For \aggview, the formulas presented in Table \ref{tbl:query-correct} apply without modifications. \reminder{It would be better to explain the reason.}
