...
\begin{abstract}
To speed up query processing the use of materialized views, pre-computing results, is becoming increasingly popular.
Incrementally maintaining these results when the base tables update has been well studied but this process can be very costly.
To defer these costs, materialized views are often updated periodically leading to periods where queries issued to the views return stale results.
