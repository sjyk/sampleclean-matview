...
In particular, we construct a small representative sample of updates to a materialized view and infer how these updates affect a query on a the stale view.
For common aggregate queries (SUM, COUNT, AVG), we can derive a correction from the sample to ``clean" the stale query result.
...
As sampling can be senstive to long-tailed distributeions, we further consider an outlier indexing technique to give increased accuracy when the data distributions are skewed.
...
We found that maintaining a 5\% sample was 16.3x faster than full incremental maintenance yet only with mean query error of 1.2\%.
\end{abstract}
