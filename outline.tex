...
\begin{abstract}
Materialized views (MVs), stored pre-computed results, are widely used to facilitate fast queries on large datasets. 
However, when base data is changed, MVs become stale requiring potentially expensive updates (maintenance).
We propose \svcfull (\svc) which addresses the expense of MV maintenance from a data cleaning perspective since staleness, like dirty data, leads to incorrect, missing, and superfluous rows.
We take inspiration from recent results in data cleaning that greatly improve query accuracy by cleaning only a small sample of dirty data.
...
In \svc, we efficiently clean data error in a sample of rows from a stale MV, and use the clean sample to correct incorrect query results on the stale view.
Since we only clean a sample, both theoretically and empirically, we show that \svc is much less expensive than full view maintenance.
While approximate, the corrected query results reflect the most recent data and our framework supports a wide variety of materialized views and aggregate queries; with optimality for SUM, COUNT, AVG.
...
SVC complements existing deferred maintenance approaches by giving accurate and bounded query answers between maintenance periods.
We evaluate our method on real dataset of server logs from Conviva and the TPCD benchmark dataset and our experiments confirm our theoretical result: (1) cleaning an MV sample is more efficient than full view maintenance, (2) the corrected results are more accurate than using the stale MV, and (3) SVC can be efficiently integrated with deferred maintenance. 
\end{abstract}
