...
When new records arrive at a fast rate, it is infeasible to continuously update (maintain) MVs and a common solution is to defer maintenance by batching updates together.
Between batches, the MV is stale with incorrect, missing, and superfluous rows leading to inaccurate query results.
...
In \svc, we efficiently clean a sample of rows from a stale MV, and use the clean sample to compute a correction for incorrect query results.
...
\svc complements existing deferred maintenance approaches by giving accurate and bounded query answers between maintenance.
