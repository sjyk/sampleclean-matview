...
\item $\Pi_{p,(a_2,...,a_k)}(R)$: \reminder{The project has a different definition than that in Sec 3.1 and 4.2.} Propagates upwards if $A$ is in the projection, otherwise null \reminder{null?}.\vspace{-.45em}
\item $\bowtie_{\phi (r1,r2)}(R_1,R_2)$: New outlier index where $A=A_{r1} \cup A_{r2}$ and $O = \bowtie_{\phi (r1,r2)}(O_1,O_2)$.
\item $\gamma_{e}(R)$: New outlier index where $A=\{e\}$ and $O = \gamma_{e}(O)$.\vspace{-.45em} \reminder{Suppose $r\in O$ is an outlier of the base relation. How can you guarantee that $O$ includes all the records that share the same group-by key with $r$.}
\item $R_1 \cup R_2$: Propagates if $A_1 = A_2$, in which case $O = O_1 \cup O_2$. \reminder{This is not very intuitive. Why do you need the condition of $A_1 = A_2$?}
...
For a given aggregation query \reminder{Does it hold for all the aggregation queries that we discussed in Sec 5?}, let $N$ be the count of records that satisfy the query's condition and $l$ be the number of outliers that satisfy the condition.
