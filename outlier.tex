...
Using the push up rules, we first push the index through the join of \tbl{Log} and \tbl{Video}.
Then, we hit the group by aggregate, where select all the distinct groups (videos) for which 
