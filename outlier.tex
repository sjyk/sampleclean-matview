...
Define an outlier index to be a triple of a set of attributes, relation, and a set of records $(A,R,O)$. The outlier index propagates upwards with the following rules: \reminder{I am a little confused about $R$. Does it represent the full relation. For example, in $\bowtie_{\phi (r1,r2)}(R_1,R_2)$, what do you mean by $R_1 \bowtie R_2$? Are you going to join two full tables? }
...
\item $\bowtie_{\phi (r1,r2)}(R_1,R_2)$: Push upwards with new outlier index $(A_{r1} \cup A_{r2}, R_1 \bowtie R_2 ,O_1 \bowtie O_2)$. \reminder{$A_{r1}$ --> $A_{1}$, $A_{r2}$ --> $A_{2}$}
\item $\gamma_{f,e}(R)$: Recall our earlier notation where the resulting schema of the group-by aggregation was $(e_i,f_i)$. For group-by aggregates, we set $A = \{f_i\}$ and $R = \gamma_{f,e}(R)$. For the outlier index, we do the following steps. (1) Apply the aggregation to the outlier index $\gamma_{f,e}(O)$, (2) for all distinct $e_i$ in $O$ select the row in $\gamma_{f,e}(R)$ with the same $e_i$, and (3) this selection is the new set of outliers $O$. \reminder{Revise it according to the new notation.}
\item $R_1 \cup R_2$: Push up with a new outlier index $(A_1 \cap A_2 ,R_1 \cup R_2,O_1 \cup O_2)$.\reminder{$(A_1 \cap A_2$ --> $(A_1 \cup A_2$?} 
...
Given these rules, $O$ is, in fact, a subset of our materialized view $S'$.\reminder{How about a record that is in both $O$ and $S'$?}
...
Since $O \subset S'$, our sample is actually a sample restricted to the set $\hat{(S'-O)}$. \reminder{$\hat{(S'-O)}$ or $(S'-O)$. If it is $\hat{(S'-O)}$, $\hat{-}$ looks like the correspondence subtract. You can use $\widehat{(S'-O)}$.}
...
For a given query, let $c_{reg}$ be the correction calculated on $\hat{(S'-O)}$ \reminder{The same issue as above} using the technique proposed in the previous section and adjusting the sampling ratio $m$ to account for outliers removed from the sample.
