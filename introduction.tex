...
Materialized Views (MVs), or pre-computed query results, are an extensively studied approach to reduce query latency on large data \cite{LarsonY85, gupta1995maintenance, chirkova2011materialized}. 
MVs are now supported by all major commercial vendors and have been successfully applied in linear algebra and machine learning \cite{nikolic2014linview, zhang2014mat}.
The key challenge in using MVs is keeping them up-to-date as base data changes.
...
The challenge with eager maintenance is that every incoming transaction requires updating all affected MVs, and thus, each additional MV reduces the available transaction throughput.
...
Other than an educated guess based on past trends, in general, the user has no way of knowing how incorrect their query results are.
