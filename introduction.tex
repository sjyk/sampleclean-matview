...
An increasingly popular approach for fast query processing on large datasets pre-computing query results \reminder{The sentence seems to have some errors.}.
...
For frequently changing tables, eager incremental maintenance can be inefficient since every update to the base data requires updating all the dependent views; a challenge that only becomes worse in a distributed environment \reminder{I think distributed environment is our weapon to argue with dbtoaster. I would suggest to move the distributed enviroment argument after dbtoaster, and say such environment can make dbtoster become even worse.}.
Even with recently proposed optimizations such as DBToaster by Koch et al. \cite{DBLP:journals/vldb/KochAKNNLS14}, these bottlenecks can force the deferral of view
maintenance \cite{chirkova2011materialized, zhou2007lazy, DBLP:conf/sigmod/ColbyGLMT96}. \reminder{Why can these papers~\cite{chirkova2011materialized, zhou2007lazy, DBLP:conf/sigmod/ColbyGLMT96} prove your claim? }
...
\centering
 ...
 \caption{Deferred maintenance can lead to stale MVs which have incorrect, missing, and superfluous rows. In \svc, we pose this as a data cleaning problem and show that we can use a sample of clean (up-to-date) rows from an MV to correct inaccurate query results on stale views. \reminder{I replace the \emph{corrected} query results in abstract with the \emph{estimated} query results. Do you think we need to do the same thing in this figure and the introduction?}\label{sys-arch}}\vspace{-1.75em}
\end{figure}
...
Since data cleaning is often expensive, we proposed cleaning a sample of data using this sample to improve the results of aggregate queries on the full dataset. \reminder{this sentence looks werid. adding ``and" in front of ``using this ..."?}
