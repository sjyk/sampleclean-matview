...
Materialized views, stored pre-computed query results, are one of the most well-known techniques to speed- and scale-up query processing \cite{gupta1995maintenance, chirkova2011materialized, halevy2001answering}.
During the last 30 years, the research community thoroughly studied them  \cite{LarsonY85, zaharia2012resilient,lefevre2014opportunistic, bailis2014scalable, perez2014history} and all major database vendors added support for them. 
More recent work expands the developed techniques to new domains and, for example, shows remarkable results for linear algebra and machine learning \cite{nikolic2014linview, zhang2014mat}.

Yet, materialized views are not for free. 
When applied to frequently changing tables maintaining the materialized view can be expensive; every update to one of the {\em base} tables requires updating all the dependent views. 
One of the most well-known technique to reduce the overhead is incremental view maintenance \cite{gupta1995maintenance, chirkova2011materialized}.
...
The results show that to answer aggregate queries on dirty datasets, it suffices to clean a small, representative sample of dirty records.
This model raises a new possibility for materialized views, namely, that a sample of up-to-date rows can be used to answer aggregate queries without incurring the cost of full view maintenance.
