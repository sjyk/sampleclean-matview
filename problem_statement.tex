...
Any form of pre-computed, derived data encounters challenges with staleness when the physical base data changes. \reminder{Polish the paragraph to make it more understandable. Right now, it contains many terminology (e.g., logical tables, physical base relations, base data)}
...
\reminder{I would suggest not to use deletion as an example because deletion is not very common in many applications. Maybe you can explain this point using some aggregation functions like median, disctinct count, etc. }
...
Along the lines of this example, especially when considering views with non-distributive aggregates and non-monotone operations \reminder{Never defined. Either remove or add references or give examples}, there are 
cases when recomputation may require less storage of state or even less computation.

So materialized views are maintained either with incremental maintenance, recomputation, or a mix.
...
A maintenance strategy $\mathcal{M}$ is a function of the database $\mathcal{D}$, $S$, and all the delta relations $\{\Delta R_i\} \cup \{\nabla R_i\}$ that returns a relational expression the execution of which updates the view. \reminder{the last sentence reads weird. }
\end{definition}


\reminder{It may be more helpful to show a maitenance stategy for an example view.}
...
\reminder{I think it's better to shorten this section. It reads a little repetive with introduction.}
...
\reminder{I changed ``contributions" to ``new challenges" because we have talked about contributions in the Introduction. Another suggest is to reduce details and give high-level ideas about what the new challenges are. }
