...
Video(videoId$\textrm{,}$ ownerId$\textrm{,}$ language$\textrm{,}$ duration)
\end{lstlisting}
...
The \tbl{Video} stores each video with the primary key (\texttt{videoId}), a number identifying the owner of the view (\texttt{ownerId}), language (\texttt{language}), and the video duration (\texttt{duration}).
...
To avoid repeatedly performing the join and visit aggregation, we could define the following MV: \reminder{I am a little confused about this example. In the video table, videoId is the primary key. Why do you need to put ownerId, language, and duration in the group-by clause?}
\begin{lstlisting}[mathescape,basicstyle={\scriptsize}]
...
\end{lstlisting}
Our queries of interest are aggregates on this view, are of the form: \reminder{Does this sentence have a syntax error?}
\begin{lstlisting}[mathescape,basicstyle={\scriptsize}]
SELECT agg(visitCount) FROM visitView 
WHERE Condition(*)
\end{lstlisting}

As \tbl{Log} table grows, this MV becomes stale, and let us denote the insertions and deletions to the table as: \reminder{It seems that you only use the LogIns table in the following examples (e.g., Fig 2). }
\begin{lstlisting}[mathescape,basicstyle={\scriptsize}]
