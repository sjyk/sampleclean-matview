...
\reminder{Replace Materialized View with MV in the following text?}
...
%We found that these two approaches tradeoff statistical efficiency and robustness to error; as \rsc only depends on clean data it is robust to the magnitude of data error and as \nsc depends on the delta of the data cleaning it performs well when the data cleaning is sparse. 
...
\centering
 ...
 \caption{A basic overview of SampleClean. SampleClean uses a random sample of dirty data to learn how a data cleaning algorithm affects queries on the sample. We can then derive a correction to compensate for the dirtiness.\reminder{We need to discuss what's the best way to show this figure.}\label{sc}}\vspace{-1.75em}
\end{figure}
...
In this work, we give an explicit treatment of non-erroneous outliers records. \reminder{What do you mean by non-erroneous?}
